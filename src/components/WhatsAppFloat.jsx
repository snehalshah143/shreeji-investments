  ...
  const contacts = [
    {
      ...
      message: 'Hi! I am interested in your financial advisory services.'
    },
    {
      number: '9632359042',
      ...
      {isExpanded && (
        <div className="mb-4 space-y-3">
          {contacts.map((contact, index) => (
            <div
              key={index}
              className="bg-white rounded-lg shadow-lg p-4 max-w-xs animate-fadeInUp"
            ...
            >
              <div className="flex items-center space-x-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-600 text-xs">+91 {contact.number}</p>
                </div>
              </div>
              <button
                onClick={() => handleWhatsAppClick(contact.number, contact.message)}
                className="w-full mt-3 bg-green-500 hover:bg-green-600 text-white text-sm font-medium py-2 px-4 rounded-lg transition-colors duration-200"
              >
                Chat on WhatsApp
              </button>
            </div>
